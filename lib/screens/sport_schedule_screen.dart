...
class _SportScheduleScreenState extends State<SportScheduleScreen>
  ...
  final Map<String, String> _preliminaryMatchFormatByDivision =
      {}; // division -> match format ('1game' or 'bestof3')
  final Map<String, bool> _hasShownGamesPerTeamDialogByDivision =
      {}; // Track dialog per division
  final Map<String, bool> _hasShownCustomSchedulePromptByDivision =
      {}; // Track if custom schedule prompt has been shown per division
  ...
  String get _preliminaryMatchFormat =>
      _preliminaryMatchFormatByDivision[_selectedDivision ?? 'all'] ?? '1game';
  ...
  bool get _allPreliminaryGamesCompleted {
    ...
    for (var match in preliminaryMatches) {
      ...
      if (matchFormat == 'bestof3') {
        // For best of 3, check if a team has won 2 games
        int team1GamesWon = 0;
        int team2GamesWon = 0;
  ...
  void _showGamesPerTeamDialog({
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        ...
        String selectedMatchFormat = _preliminaryMatchFormat;
                      ...
                      children: [
                        Expanded(
                          child: GestureDetector(
                            onTap: () => setDialogState(() => selectedMatchFormat = '1game'),
                            child: Container(
                              padding: EdgeInsets.symmetric(vertical: 10),
                              decoration: BoxDecoration(
                                color: selectedMatchFormat == '1game'
                                    ? Color(0xFF2196F3)
                                    : Colors.grey[300],
                        ...
                        Expanded(
                          child: GestureDetector(
                            onTap: () => setDialogState(() => selectedMatchFormat = 'bestof3'),
                            child: Container(
                              padding: EdgeInsets.symmetric(vertical: 10),
                              decoration: BoxDecoration(
                                color: selectedMatchFormat == 'bestof3'
                                    ? Color(0xFF2196F3)
                                    : Colors.grey[300],
                                borderRadius: BorderRadius.circular(6),
                              ),
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    final matchFormatChanged = selectedMatchFormat != _preliminaryMatchFormat;

                    // Only show reset dialog if settings are actually being changed
                    // and this is not the first time setting them up
                    if ((gamesChanged || scoreChanged || matchFormatChanged) && !isFirstLoad) {
                                ...
                                onPressed: () async {
                                  ...
                                  setState(() {
                                    ...
                                    _preliminaryGameWinningScoreByDivision[_selectedDivision ??
                                            'all'] =
                                        selectedScore;
                                    _preliminaryMatchFormatByDivision[_selectedDivision ??
                                            'all'] =
                                        selectedMatchFormat;
                                    _matchesCache.clear();
    ...
    try {
      ...
      if (mounted) {
        ...
        setState(() {
          ...
          _preliminaryMatchFormatByDivision[_selectedDivision ?? 'all'] =
              preliminarySettings['matchFormat'] ?? '1game';
      ...
      children: [
        ...
        Builder(
          builder: (context) {
            ...
            final sortedTeamIds = _teams.map((t) => t.id).toList()..sort();
            final customCacheKey = '${widget.sportName}_${division}_${sortedTeamIds.join('_')}_custom';
            final bool hasCustomSchedule = _matchesCache.containsKey(customCacheKey);
            
            // Hide Reshuffle button if custom schedule is active
            if (hasCustomSchedule && !_allPreliminaryGamesCompleted) {
              return const SizedBox.shrink();
            }
  ...
  void _showPreliminaryScoringDialog(Match match) {
    ...
    // For best of 3, use PlayoffScoringScreen like SF
    if (matchFormat == 'bestof3') {
      // Get initial scores from match
      final initialScores = <String, dynamic>{};
      for (int game = 1; game <= 3; game++) {
        final team1GameScore = _getGameScore(match.id, team1Id, game) ?? 0;
        final team2GameScore = _getGameScore(match.id, team2Id, game) ?? 0;
        initialScores['${team1Id}_game$game'] = team1GameScore;
        initialScores['${team2Id}_game$game'] = team2GameScore;
      }
      
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => PlayoffScoringScreen(
            match: match,
            initialScores: initialScores,
            matchFormat: 'bestof3',
            gameWinningScore: minScore,
    ...
    // For 1 game format, use the existing dialog
    final team1Score = _getGameScore(match.id, team1Id, 1) ?? 0;
    final team2Score = _getGameScore(match.id, team2Id, 1) ?? 0;
  ...
  void _showCustomSchedulePrompt(int selectedGames, int selectedScore) {
    ...
    // Check if prompt has already been shown for this division
    if (_hasShownCustomSchedulePromptByDivision[division] == true) {
          ...
          actions: [
            TextButton(
              onPressed: () async {
                ...
                _hasShownCustomSchedulePromptByDivision[division] = true;
  ...
  // Show custom schedule screen
  void _showCustomScheduleDialog(int selectedGames, int selectedScore) async {
    ...
    final createdMatches = await Navigator.of(context).push<List<Match>>(
      MaterialPageRoute(
        builder: (context) => CustomScheduleScreen(
          allTeams: allTeams,
          division: division,
          sportName: sportName,
          selectedGames: selectedGames,
          selectedScore: selectedScore,
          onScheduleCreated: (matches) {
    ...
    // Refresh UI if matches were created
    if (createdMatches != null && mounted) {
      setState(() {});
    }
  }
}
                ...
                Expanded(
                  child: GestureDetector(
                    ...
                    child: Container(
                      padding: const EdgeInsets.all(16),
                      decoration: BoxDecoration(
                        color: _selectedMatchTotalGames == '1game'
                            ? const Color(0xFF2196F3)
                            : Colors.grey[200],
                ...
                Expanded(
                  child: GestureDetector(
                    ...
                    child: Container(
                      padding: const EdgeInsets.all(16),
                      decoration: BoxDecoration(
                        color: _selectedMatchTotalGames == 'bestof3'
                            ? const Color(0xFF2196F3)
                            : Colors.grey[200],
                        borderRadius: BorderRadius.circular(10),
                      ),
                ...
                Expanded(
                  child: GestureDetector(
                    ...
                    child: Container(
                      padding: const EdgeInsets.all(16),
                      decoration: BoxDecoration(
                        color: _selectedGameWinningScore == 11
                            ? const Color(0xFF2196F3)
                            : Colors.grey[200],
                        borderRadius: BorderRadius.circular(10),
                      ),
                ...
                Expanded(
                  child: GestureDetector(
                    ...
                    child: Container(
                      padding: const EdgeInsets.all(16),
                      decoration: BoxDecoration(
                        color: _selectedGameWinningScore == 15
                            ? const Color(0xFF2196F3)
                            : Colors.grey[200],
                        borderRadius: BorderRadius.circular(10),
                      ),
